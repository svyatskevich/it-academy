...
describe("Search text", () => {
   ...
   it(`Should contain search text "${TEXT.APPLE}" in 1st link of search results`, () => {
      ...
      globalNavSubMenu.invokeTextVisible(globalNavSubMenu.getSearchResultLink(1)).should("contain", TEXT.APPLE);
   });

   it(`The search text '${TEXT.APPLE}' on Home page should be contained in search input on Search page`, () => {
      globalNavSubMenu.goToSearchResultPage(TEXT.APPLE);
      searchPage.invokeVal(searchPage.searchInput).should("eq", TEXT.APPLE);
   });
   ...
   it(`Should contain searh text "${TEXT.WATCH}" in 1st result link in Explore list`, () => {
      ...
      searchPage.invokeTextVisible(searchPage.getExploreListItem(1)).should("contain", TEXT.WATCH);
   });
});
